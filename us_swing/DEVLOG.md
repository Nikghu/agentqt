...
- Artifacts updated: ISS (GUI-0001 first GUI issue, EXE-0012), SRD (new GUI-006.018, user-approved then Implemented), UTCD (UT-GUI-006.018.M01.T01-T06, UT-EXE-006.001.M01.T20-T21), TRACE (GUI v1.10.1, EXE v1.20.2), RN, Code, Tests, TODO (T26-T28), CONTEXT, DEVLOG
...
- Follow-up in the same session: the user asked for the three remaining Yahoo call sites (ISS-INF-0003, RN-INF-1.7.0-20260904). Rather than copy the one-liner again, `yahoo_symbol()` moved into a new `core/symbols.py` under new `SRD-INF-007.006` / `MD-INF-007.001.M03` (both user-approved then Implemented), so no tool reaches into another and the next vendor call site has a requirement to follow. All four sites convert now: the candle loader, market-watch quotes (`app_service.py`), universe market caps (`universe/store.py`) and the live batch download (`live_bar_worker.py`). The last one needed the per-symbol frame lookup converted as well — `yf.download(group_by='ticker')` keys columns by the symbols as sent, so converting only the request would have swapped one silent miss for another. Every one of these sites swallows failure into a zero, a None or a continue, which is why the defect was invisible. Left alone deliberately: `swing_trader_data.py` (takes arbitrary user tickers, no canonical dotted form) and the still-duplicated IBKR notation (TODO T29). 5 new tests (UT-INF-007.006.M03.T01-T05), all three call-site cases confirmed failing pre-fix; 748 passed; ruff 19 and mypy 147 unchanged from baseline.
- Open: `DatabaseManager` still has no `dispose()`/`close()` (TODO T28). IBKR notation still copy-pasted at three sites (TODO T29).
- Closed after review: `SRD-INF-007.006` approved by the user on 2026-09-04 and set to `Implemented`, together with `MD-INF-007.001.M03` and the INF TRACE row.
- Closed after review: `SRD-GUI-006.018` approved by the user on 2026-09-04 and set to `Implemented` — it is the first SRD to govern the Reset Database button (TODO T26).
