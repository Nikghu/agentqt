...
| SRD-INF-007.006 | FO-INF-007 | Must | Every outbound market-data call converts the canonical dotted symbol into the vendor's class-share notation; stored rows, logs and lookups keep the dotted form. | canonical symbol (e.g. `BRK.B`) | vendor symbol (Yahoo `BRK-B`, IBKR `BRK B`) | Conversion is per-call and never persisted; a vendor without class shares is unaffected (ISS-EXE-0012) | Draft |
