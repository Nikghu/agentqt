...
**Document ID:** MD-EXE
**Version:** 1.14.0
**Traces To:** SRD-EXE v1.21.0 / DD-EXE v1.15.0
**Status:** Draft
**Last Updated:** 2026-06-17
**Project:** US Swing Trading System

> v1.13.0: MD-EXE-006.013.M01 added + MD-EXE-011.001.M03 updated (ISS-EXE-0009) — opening a trade cycle arms candle feeds for its symbol in `app_service.py`; `_evaluator.py` raises `EvaluatorError` on a NaN indicator result.
...
## Module Modifications for FO-EXE-006 (ISS-EXE-0009)

Opening a trade cycle must arm the candle feeds for its symbol, symmetric with the tick feed.

| Module ID | File | Change Required | SRD |
|---|---|---|---|
| MD-EXE-006.013.M01 | `src/us_swing/gui/app_service.py` | Extend `_on_cycle_symbols_changed`: after the existing `_sync_tick_subscriptions()`, compute just-opened symbols not already in `_filtered_symbols` and emit a new queued `_arm_candle_feeds_requested(object)` signal. Add the GUI-thread slot `_arm_candle_feeds(uncovered: list[str])` — unions them into `_filtered_symbols`, calls `_start_intraday_loader(uncovered)` (delta-fetch, idempotent), and `LiveBarWorker.set_symbols(_filtered_symbols)` when a worker is running. Marshalled to the GUI thread because the callback fires on the `TradeCycleService` background thread (DD-EXE-006.013.D01). | SRD-EXE-006.013 |
...
| MD-EXE-011.001.M03 | SRD-EXE-011.006, .023 | `src/us_swing/execution/strategy_engine/_evaluator.py` | `ConditionEvaluator` — tokenizer + recursive-descent parser + `FUNCTION_MAP` of 14 indicators; stateless; evaluates expression against candle dict. `_eval` FUNC branch raises `EvaluatorError` on a NaN indicator result so an un-computable condition surfaces instead of silently reading as false (ISS-EXE-0009). | `evaluate(expr, candles, symbol) -> bool` | `pandas`, `numpy`, `talib`, `math`, `re` | No | Approved |
| MD-EXE-011.001.M04 | SRD-EXE-011.008, .009, .011, .012, .021, .022 | `src/us_swing/execution/strategy_engine/_router.py` | Signal-queue consumer + Mode/auto_trade dispatch; calls `RiskManager.validate()` and `ExecutionRouter.submit()` for auto path or `PendingSignalStore.add()` for manual path; owns `_end_time_watcher_loop`; `_open_cycle_qty`/`_open_cycle_price` read held qty + last price for exit signals | `_router_loop()`, `_force_exit(ctx, symbol, reason)` (coroutines, internal) | `risk_manager`, `execution_router`, `pending_signal_store`, `_signals`, `_events` | No | Approved |
...
| — | `src/us_swing/execution/pending_signal_store.py` | **NEW** — `PendingSignalStore`: in-memory `dict[str, TradeSignal]` keyed by `signal_id`, with `add()`, `dismiss()`, `dismiss_for(strategy_id, symbol, action)` (ISS-EXE-0010), `execute()`, `list()`, signals `pending_signal_added(signal)` / `pending_signal_removed(signal_id)`. Consumed by FO-GUI-014 Active Cycles Panel. | SRD-EXE-011.009, .024 |
| MD-EXE-011.024.M01 | `src/us_swing/gui/app_service.py` | ISS-EXE-0010: subscribe to `CycleClosed` and clear matching pending exit signals via `dismiss_for` (marshalled through `_clear_pending_exits_requested` → `_clear_pending_exits`); `execute_signal` rejects a pending `EXIT` whose `(strategy, symbol)` has no `OPEN` cycle. | SRD-EXE-011.024, .025 |
...
| MD-EXE-017.012.M10 | `src/us_swing/execution/risk_manager.py` | Add `margin_available() -> float` (effective capital − all-strategy deployed − reservations) and a `dict[(strategy,symbol), float]` reservation ledger with `reserve()`/`release()`. Remove unused `can_enter_new`. | SRD-EXE-017.015, .017 |
| MD-EXE-017.013.M11 | `src/us_swing/execution/strategy_engine/_protocols.py` | Extend `RiskValidator` with `margin_available()`, `reserve(strategy_id, symbol, value)`, `release(strategy_id, symbol)`. | SRD-EXE-017.015, .017 |
| MD-EXE-017.014.M12 | `src/us_swing/execution/strategy_engine/_router.py` | After `can_allocate`, clamp entry qty to `floor(margin_available()/price)`; drop `reason='margin_exhausted'` + edge WARNING when `<1`; `reserve()` on commit; `release()` in `on_order_fill` (entry), `_rollback`, `on_order_reject`. | SRD-EXE-017.016, .018 |
| MD-EXE-017.015.M13 | `src/us_swing/execution/strategy_engine/_context.py` | Add `margin_warned: bool` edge-trigger flag (mirrors `capital_warned`). | SRD-EXE-017.016 |
| MD-EXE-017.016.M14 | `src/us_swing/gui/app_service.py` | `get_account_state` sums paper `open_position_value` from `_cycle_position_source`; add `margin_available()`; live `_reconcile_margin_drift()` (one-per-crossing latch) in `_on_account_data_ready`. | SRD-EXE-017.019, .020, .021 |
| MD-EXE-017.017.M15 | `src/us_swing/gui/main_window.py` | `_AdminContextBar` adds a `capital` cell rendering `Cap $X · Avail $Y` from `effective_capital()`/`margin_available()`; green when `Y>0`; refresh on `account_updated`. | SRD-EXE-017.021 |
| MD-EXE-017.023.M03 | `src/us_swing/execution/strategy_engine/_router.py` | In `_size_entry`, multiply the strategy allocation by `per_trade_pct/100`: `budget = eff_cap × capital_max × per_trade_pct / 10000`. `can_allocate` (full-allocation cap) unchanged, so a strategy may hold up to `floor(100/per_trade_pct)` concurrent positions. | SRD-EXE-017.024 |
