...
| UT-EXE-017.024.M03.T04 | MD-EXE-017.023.M03 | Positive | Per-trade size is allocation × per_trade | `_size_entry` with eff_cap=$2000, capital_max=40%, per_trade_pct=25%, entry=$100 | `2` (budget $200) | Pass |
| UT-EXE-017.024.M03.T05 | MD-EXE-017.023.M03 | Edge | `per_trade_pct=100` reproduces full-allocation sizing (back-compat) | `_size_entry` with eff_cap=$2000, capital_max=25%, per_trade_pct=100, entry=$96 | `5` | Pass |
| UT-EXE-017.024.M03.T06 | MD-EXE-017.023.M03 | Positive | Smaller per-trade leaves allocation room for a second entry | eff_cap=$2000, capital_max=100%, per_trade_pct=25%, $500 already deployed | `_size_entry → 5` and `can_allocate(ok=True)` | Pass |
| UT-EXE-017.024.M03.T07 | MD-EXE-017.023.M03 | Negative | Per-trade slice too small to afford one share drops the entry | `_size_entry` with eff_cap=$2000, capital_max=5%, per_trade_pct=5%, entry=$100 (budget $5) | `0` → router drops `StrategySignalDropped(reason='capital_insufficient')`, no enqueue | Pass |
