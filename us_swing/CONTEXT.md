...
**Current (Session 59, 2026-06-04):** **Closed out the Broker_fix / cleanup line and delivered FO-EXE-016 (retire `positions` table; OrderIngestion-driven monitoring lifecycle).** Five merged PRs. (1) **Broker legacy cleanup** (`cleanup.md` Steps 1–6, PRs #30 + #32) — removed the dead paper path: `app_service` PaperBroker fallback + `_on_paper_fill`/`_record_paper_*`, then `paper_broker.py`, `execution_router.py`, `paper_engine.py`, `execution_engine.py`, `position_tracker.py` (+ tests); de-coupled `risk_manager` via a local `_PositionSource` Protocol. Plan-order bug caught: `execution_router` imports `PaperEngine`, so router (true top leaf) deleted before PaperEngine. (2) **Delivery-gap fix (PR #31)** — discovered the entire broker-abstraction + a strategy-store refactor were code-complete but **never committed**; cleanup PR #30 had merged an `app_service` importing those untracked modules, briefly breaking `main`. Delivered the full consistent snapshot (entangled in `_engine.py`). (3) **Health repoint (Step 7A, PR #33)** — `HealthCheck` open-position count → `trade_cycles`. (4) **FO-EXE-016 (PR #34)** — full FO→SRD→DD→MD→Code→RN chain (UTCD deferred = DoD debt). cleanup.md Steps 7B–8 were **re-scoped** into this FO after finding `on_fill` is the *designed* Lifecycle hook in `Final_Execution.md` §2.5–2.6, not dead legacy. Result: `positions` table RETIRED (`trade_cycles` is the sole open-position surface); `MONITORING→ENTERED→EXITED` now driven by `OrderIngestion` fills via new idempotent `mark_entered`/`mark_exited` (replaced unwired `on_fill`); `open_system_position_symbols` reads non-terminal `trade_cycles`; `db/manager` positions methods + `db/schema` positions table dropped (idempotent `DROP TABLE IF EXISTS positions` in `migrate_lifecycle_columns`). `PositionRecord`/`OpenPosition` data models KEPT (GUI IBKR portfolio). SRD-EXE-016.001–.006 + SRD-EXE-015.006 → Implemented; TRACE-EXE 1.13→1.14; RN-EXE-1.18.0-20260604. Manual GUI paper-trade verify PASSED (entry→exit round-trip via the new pipeline, confirmed in `~/.usswing/logs`). Suite held at the **32-failure pre-existing baseline** throughout; ruff clean, no new mypy errors. **Next:** UTCD backfill for FO-EXE-016 (DoD debt); FO-EXE-003 (CircuitBreaker + EmergencyShutdown).
