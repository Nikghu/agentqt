...
**Current (Session 79, 2026-09-04):** **Reset-Database crash + Yahoo dotted-symbol fallback (Bugfix, GUI/EXE; ISS-GUI-0001, ISS-EXE-0012; RN-GUI-1.4.1-20260904, RN-EXE-1.32.3-20260904).** The user reported a crash log. Two unrelated defects behind it. (1) **GUI** — Settings -> Database -> Reset Database died on `PermissionError [WinError 32]`: `reset_candle_db()` unlinked `~/.usswing/candles.db`, which five components hold open for the app's lifetime, and the exception escaped the unguarded Qt slot. Reading the live DB showed the deeper fault — that file also stores 133 trade cycles, 268 trades, 88 rex counters and 2 strategies, so the delete was never safe to succeed; the Windows lock is the only reason that data was never lost. Reset now runs `DELETE FROM price_1d / price_1w` in place (scope confirmed with the user: daily + weekly only), keeps every trading and intraday table, never touches the file, and `VACUUM` is best-effort. `_on_reset_db` now catches, logs, shows a `QMessageBox.critical` and always restores the button; the confirm dialog was reworded to match. 6 regression tests. New `SRD-GUI-006.018` written, **approved by the user on 2026-09-04** and set to `Implemented`. (2) **EXE** — `_fetch_symbol_yfinance()` passed the canonical dotted symbol to `yf.Ticker`, but Yahoo keys class shares with a hyphen, so `BRK.B` and `BF.B` returned "possibly delisted" on every fallback run. New `_yahoo_symbol()` converts the lookup key only; stored rows stay dotted. 2 regression tests. 141 passed across `tests/gui/` + the loader suite; ruff and mypy counts unchanged from baseline. (3) **INF** — the user then asked for the three remaining Yahoo call sites (ISS-INF-0003; RN-INF-1.7.0-20260904). `yahoo_symbol()` moved into new `core/symbols.py` under new `SRD-INF-007.006` / `MD-INF-007.001.M03`, and all four sites now convert: market-watch quotes, universe market caps, and the live 3m/15m batch download — the last needing the frame lookup converted too, since `yf.download(group_by='ticker')` keys columns by the symbols as sent. 5 more tests; 748 pass. `SRD-INF-007.006` **approved by the user on 2026-09-04** and set to `Implemented`.

**Previous (Session 78, 2026-08-28):** **IBKR live-readiness review + GUI submit-failure guard (Bugfix, EXE/GUI; SRD-EXE-015.004; MD-EXE-015.004.M01; PR #68).** Reviewed the whole live path against `IBKR_Live_Execution_Plan.md` before the paper smoke test: every Phase 0–4 change is actually in the code, `broker/` + `execution/` are ruff- and mypy-clean, and the 11 remaining suite failures are all in `analysis/test_candle_builder.py` and `screener/test_preset.py` — nothing on the execution path. Found one real gap. The Phase 4 liveness gate raises `BrokerConnectionError` and `BrokerAdapter.submit` re-raises it; the router catches that and rolls back, but **both GUI order paths called the submitter bare** — `execute_signal:2141` and `_submit_cycle_exit:2488` — so the exception reached the Qt slot at `execution_panel.py:1488`. Worse, `execute_signal` pops the signal from `_pending_store` **before** submitting and the store has no restore path, so a failed submit lost the row with no way to retry. That is exactly step 5 of the smoke-test runbook (kill TWS mid-flight). Both paths now catch, log a user-facing `[Orders]` message and return `-1`; `execute_signal` re-adds the popped signal. Deliberately left alone: `_submit_cycle_exit` sets `_pending_exit_reason` before submitting, so a failed submit leaves a stale reason for a later unrelated exit fill — separate from this guard. Also confirmed while reviewing: `cancel_order` has **no production caller**, so the CANCELLED branch only fires on a manual TWS cancel; `_order_ids` in `IBKRClientGateway` is never pruned (noise, not harm); the broker is built once from `_users[0].mode`, and Settings already warns to restart after a mode change. Tests: 5 new (`UT-EXE-015.004.M01.T32–T36`), gui+execution+broker **385 passed**; ruff unchanged from baseline, mypy clean on the edited regions. **Phase 4 live smoke test on DU7078110 still NOT RUN** — runbook committed, this was the last code gap before it.

**Previous (Session 77, 2026-08-27):** **IBKR live execution Phases 1–4 + F4 + 21-test revival (Bugfix, EXE/INF/GUI; RN-EXE-1.32.0-20260827; PRs #60–#65).** Closed the rest of `IBKR_Live_Execution_Plan.md`. One through-line: an order that did not fill cleanly stranded its symbol `in_flight` with capital reserved and nothing ever coming to free it. **Phase 1 (#60):** `RejectEvent` had a fully-built consumer chain and **zero producers** — `order_ingestion` now has a required `reject_sink` + `_notify_reject()`, and the router rolls back on a dispatch exception. Two deliberate deviations: one sink not two (identical lambdas), and required not optional (an optional sink silently does nothing — the exact failure being removed). **Phase 2 (#61):** four hardcoded `"paper"` values → the active user's mode; fixing `mode_provider` fixes the ledger end-to-end via `OrderContext.mode` → `TradeRecord.mode`. TODO T7 **closed not actioned** — `insert_trade_with_anchor` no longer exists. **Phase 3 (#62):** `PendingCancel` was terminal so it popped `client_ref` and a racing fill arrived unattributable; margin rejects arrive **only** via `errorEvent` which nobody watched (Phase 1 built the path, no event existed to send down it); `reason` was hardcoded `""`. Codes 103/201→reject, 202→cancel, all else ignored as account-wide chatter. **Phase 4 (#63):** `_require_live()` raises `BrokerConnectionError` before touching the socket, feeding Phase 1's rollback. **Order pacing deliberately skipped** — `PacingQueue` is the historical-data limiter and would block a stop-loss exit for minutes. **F4 (#64):** `execute_signal` now refuses a manual ENTRY on a held stock; the harm was `_open_cycle_qty` taking the first match, so an auto-exit sold 10 and left 5. Reachable via the `fill_sink`-before-`_open_cycle` window. **Tests (#65):** revived all 21 stale failures — Market Watch moved to ETFs, `set_contracts` became a router, candle fixtures had date-decayed against a 30-day window with timeframes changed to 3m/15m, and 4 indicator stubs were removed. Suite: **380 passed, 0 failed** (was 357/21). ruff and mypy at or below baseline throughout. **Outstanding:** the **Phase 4 live smoke test is NOT RUN** — deferred by the user, runbook at `docs/execution/Phase4_Live_Smoke_Test.md`; plus F5 (DB unique index, needs a migration), N1 (silent cancel no-op after reconnect), N2 (`cancel_all_orders`/`close_all_positions` are account-wide, no callers yet), the `_users[0].mode` multi-user caveat, and SRD-GUI-013.006 documenting 15 indicators where 10 exist. **Next:** run the live smoke test, then Phase 5 (dashboard order routing) or FO-EXE-003.

**Previous (Session 76, 2026-08-26):** **Live tick worker watchdog — LiveTickWorker no longer dies once (Bugfix, GUI; FO-GUI-012 / FO-EXE-008; SRD-GUI-012.008–009; RN-GUI-1.4.0-20260826).** Diagnosed from `~/.usswing/logs/`: TWS had the API port open but refused every handshake with **error 10141 (paper-trading disclaimer not accepted)**. `LiveTickWorker`'s event loop returned, its QThread finished, and prices stopped for the whole 80-minute session — Market Watch blank, open HOOD cycle with LTP frozen at entry and no P&L. Cause: `_on_connect_ok` guarded creation with `if self._tick_worker is None`, and that attribute was cleared only in `disconnect_feed()`, so no reconnect ever rebuilt the worker; separately the feed retry loop stops on a **TCP probe**, not a proven API handshake, so `_on_connect_ok` was never called again. **Fix (all in `gui/app_service.py`; `live_tick_worker.py` untouched, so SRD-EXE-008.006 needed no reopen):** 30 s `_tick_watchdog` QTimer → `_check_tick_health` restarts the worker when `isFinished()` and status is still CONNECTED; worker creation extracted to `_start_tick_worker()`; `finished` → `_on_tick_worker_finished(tw)` via `partial` clears `_tick_worker` + `deleteLater()` with an identity guard; `_last_tick_at` stamped per tick and `_warn_if_ticks_stale` logs once after 90 s of silence in market hours (**warn only — never restarts**, to avoid reconnect churn); `_on_tick_sub_failed` now also emits to the GUI log panel. Rejected `reqMarketDataType(3)` — per-client global, would drop a live account to delayed prices. Tests: 9 new (UT-GUI-012.001.M01.T20–T28) all pass; gui+execution+broker = 21 failed / 298 passed, the 21 being the documented pre-existing baseline (verified identical against clean HEAD); ruff/mypy clean on every changed line. **Verified live:** TWS killed 21:29:24 → five watchdog attempts → `[Tick] Connected to IBKR` at 21:31:52, full recovery in 2 min 28 s with no app restart. **Follow-up:** `_connect_with_retry` catches `TimeoutError` in its generic `except` and returns before the clientId-increment retry runs (SRD-EXE-008.006) — masked by the watchdog, worth a separate fix. **Next:** `IBKR_Live_Execution_Plan.md` Phase 0 (T15 record-resolution audit — hard gate before any live order; dashboard mutators still not disabled in live mode). Phases 0–4 remain unimplemented apart from the dedicated order client id.

**Previous (Session 75, 2026-07-10):** **Pre-release Telegram bot polish + scheduler crash fix (Enhancement/Fix, INF+GUI; FO-INF-010; RN-INF-1.6.0-20260710).** (1) **Command menu:** `TelegramPoller._register_commands` POSTs `setMyCommands` once on poll start (best-effort); a new single `_COMMANDS` list drives both `/help` and the registration so they can't drift — Telegram now shows the tap-to-run menu + `/` autocomplete. (2) **Professional replies:** all six command answers now use `parse_mode: "HTML"` — bold titles, 🟢/🔴 status dots, monospace `<pre>` tables (positions), every dynamic value `html.escape`d; unknown-command hint escaped; `/help` gained an HTML header. (3) **`/screener` enrichment:** sorted by score, top-10 (+"…N more"), each stock shows score + last price + day % change (one `get_candles_bulk("1d", limit=2)` round-trip; "—" when no candle data) + matching preset name, with an "As of date · time" freshness header. (4) **Scheduler crash fix (`gui/scheduler_dialog.py`):** TWS auto-login window-finder crashed with `OverflowError: int too long to convert` on 64-bit `HWND`s — new `_user32()` declares proper `argtypes`/`restype` on the `user32` funcs so handles marshal as pointers; verified live on Windows (`_find_tws_hwnd()` enumerates all windows cleanly). Tests: +1 setMyCommands + parse_mode assertion (test_inbound.py, 9 pass), +1 `/screener` ranking/price/preset (test_telegram_commands.py, 4 pass); full notification+telegram suite 49 pass; ruff + mypy --strict clean on changed files. Docs: MD-INF M10 note, UTCD-INF M10.T09, RN-INF-1.6.0. Released via `release/telegram-polish-scheduler-fix` branch + PR (merged). **Follow-up:** outbound event messages (tool started/screener/day-end) still plain text — could get the same HTML treatment. **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown) still pending.

**Previous (Session 74, 2026-07-10):** **FO-INF-010 follow-ups closed — per-event toggles + day-end delivery + P&L bug (Feature/Fix, INF; SRD-INF-010.006, .010; RN-INF-1.5.0-20260710).** Finished the three remaining Telegram items. (1) **Per-event toggles enforced:** `NotificationDispatcher` gained an `event_toggles` map; `dispatch` drops any rendered message whose `event_kind` is toggled off (unknown kind defaults on, so new event types are never swallowed); `build_default_dispatcher` passes `config.event_toggles` through. Three flat `UserProfile` bools (`notify_tool_started`/`notify_screener_approved`/`notify_day_end_pnl`, default True) round-trip via `user_store`; `_build_notification_config` maps them into `notifications.events`; Settings → Telegram gained a "Notify on:" row with three checkboxes. (2) **Day-end at market close:** `_refresh_market_status` (60 s tick) calls new `_maybe_publish_day_end(today)` whenever status is `after_hours` (16:00–20:00 ET — trading days only, weekends/holidays excluded for free); a `_day_end_sent_date` guard sends once per day and is *not consumed* until `_notif_worker` is up (so a startup send is never lost). In-app only, app must be open at close (user-confirmed scope). Init reordered so `_notif_worker`/`_day_end_sent_date` exist before the immediate market-status check. (3) **Fixed** the latent `getattr(p, "unrealized_pnl", 0.0)` → `p.unrealised_pnl` (British) so day-end unrealised is no longer always 0.0. Known interaction (left pending a user decision): Send Test fires a `ToolStartedEvent`, so turning "Tool started" off silently drops the test. Tests: +4 dispatcher/factory (M04.T07–09, M07.T03), +2 user_store toggle round-trip (M08.T06–07), +3 day-end guard (new `tests/gui/test_day_end_notification.py`, M09.T01–03) = 36 notifications+telegram pass; ruff + mypy --strict clean on touched files. 9 failures in `tests/gui/test_app_service_tick.py` are **pre-existing on clean HEAD** (real Yahoo `^GSPC` data leak), unrelated. Doc bumps: MD-INF 1.4.0, UTCD-INF 1.6.0, TRACE-INF (new forward row + M04 reverse .006), RN-INF-1.5.0. Committed on a scoped `feat(inf)` branch + PR. **E2E hand-off:** still needs the user's real bot token + chat id in Settings. **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown) still pending.

**Previous (Session 73, 2026-07-09):** **FO-INF-010 inbound two-way Telegram commands (Feature, INF; SRD-INF-010.012–.015, realises .009).** Turned the reserved inbound seam into 7 working read-only commands: `/help`, `/status`, `/pnl`, `/positions`, `/signals`, `/screener`, `/cycles`. Design decisions (both user-confirmed): auto-approve SRDs + GUI-thread marshalling. New `core/notifications/_inbound.py` = `TelegramPoller` (long-poll `getUpdates` on the notification loop, tracks `offset=last_update_id+1`, reuses the worker's shared `httpx` client, runs only when Telegram enabled) + `CommandRouter` (parse → strip `/` & trailing `@botname`, lowercase; static `/help`; unknown → help hint; handler error → plain apology, never a stack trace). Chat authorization (SRD-.013): only the configured `chat_id` is answered, every other sender dropped + logged `[Notify]`. New `CommandPort` Protocol in `_protocols.py` (core seam, no GUI import) + `build_command_poller` in `__init__.py`. New `gui/telegram_commands.py` `TelegramCommandBridge(QObject)` implements `CommandPort` by marshalling each query onto the GUI thread via a queued signal + `concurrent.futures.Future` (poller stays non-blocking through `run_in_executor`; same-thread fast path avoids deadlock/tests); formats existing `AppService` reads (`get_feed_status`/`get_market_status`/`get_account_state`/`get_positions`/`get_pending_signals`/`get_latest_screener_results`/`get_recent_closed_cycles`+`get_strategies_with_open_cycles`). Worker now hosts the poller task alongside the dispatcher; `app_service._start_notifications` builds the bridge and passes it. Note: used `p.unrealised_pnl` (real British-spelling property; the existing `_publish_day_end_pnl` uses `getattr(..., "unrealized_pnl", 0.0)` which always hits the 0.0 default — pre-existing latent bug, left untouched). Dropped the speculative `CommandChannel` Protocol from the old DD sketch (avoid dead code). Full chain: SRD-INF-010.012–.015 Implemented (.009 Approved→Implemented), DD-INF-010.001.D01 updated, MD M10/M11, UTCD 11 new cases (M10 ×8, M11 ×3, all Pass), RN-INF-1.4.0-20260709. Tests: 11 new pass; 35 total notifications+telegram-settings pass (no regressions); ruff clean on all new/changed; mypy --strict clean on the 5 new/changed non-app_service files (`app_service.py` retains only its pre-existing lint/graph debt). Doc bumps: FO-INF (inbound bullets), SRD-INF 1.9.0, DD-INF (D01 range→.015), MD-INF (M10/M11), UTCD-INF 1.5.0, TRACE-INF 1.4.0. **Not committed** — working tree already carries unrelated pre-existing changes; a scoped `feat(inf)` branch+PR is the next step per the git-workflow rule. **E2E hand-off:** needs the user's real bot token + chat id in Settings to talk to the live Bot API. **Remaining follow-up (second half of the original ask):** per-event notification toggles — `NotificationConfig.event_toggles` is parsed but not enforced, and Settings has no per-event switches yet.

**Previous (Session 72, 2026-07-09):** **FO-INF-010 — Telegram Notification Integration, full FO→RN chain (Feature, INF).** Built a scalable outbound notification service end-to-end (user approved the SRD mid-session and cleared implementation). Three locked directions: (a) a **separate** business-event service, distinct from FO-INF-005's log-level `AlertDispatcher`; (b) **outbound now**, inbound two-way commands reserved as a design seam only; (c) real code this session. New package `src/us_swing/core/notifications/` (in `core/` so SCR/EXE/INF emit via a shared bus with no cross-tool imports; mirrors `core/monitoring_session/` CQRS-lite): `_events.py` frozen `NotificationEvent` variants (`ToolStartedEvent`, `ScreenerApprovedEvent(symbols)`, `DayEndPnLEvent`) + `_InProcessBus`; `_formatters.py` `FormatterRegistry` — a new notification = new frozen subclass + one `register()` call, never a dispatcher edit (the open/closed spine); `_protocols.py` `NotificationChannel`/`NotificationBus` Protocols; `_telegram.py` `TelegramChannel` (Bot API over HTTP via injected `httpx.AsyncClient`, no SDK); `_dispatcher.py` `NotificationDispatcher` — bus-subscribe, render, non-blocking enqueue, worker with per-chat rate limit + bounded retry-with-backoff + per-channel failure isolation (never crashes the caller), all logged `[Notify]`; `_dto.py` `NotificationMessage`/`NotificationConfig` + `load_config` parsing per-user `settings_json` (FO-INF-006), half-configured Telegram reads as disabled; `__init__.py` public surface + `build_default_dispatcher(config, http=...)` factory (disabled → zero channels, publish is a no-op). SRD-INF-010.009 (inbound `CommandChannel`/`CommandRouter`) stays Approved-but-unimplemented, documented in DD only. Full chain: FO-INF-010, SRD-INF-010.001–.008 Implemented (.009 Approved), DD-INF-010.001.D01, MD-INF-010.001.M01–M07, UTCD 22 cases (all Pass), 7 source files, RN-INF-1.2.0-20260709. Added `httpx>=0.27` to pyproject deps (was transitive via `openai`). Tests: 22 passed (`tests/core/notifications/test_notifications.py`); ruff clean; mypy --strict clean on all 7 files. Doc versions: FO-INF 1.5.0, SRD-INF 1.7.0, DD-INF 1.3.0, MD-INF 1.2.0, UTCD-INF 1.3.0, TRACE-INF 1.3.0. artifact-validator GO.

**Then wired the service into the running app (same session, RN-INF-1.3.0):** Since there is no app-wide asyncio loop (Qt/QThread app), the async dispatcher runs in a new `NotificationWorker(QThread)` (`gui/notification_worker.py`) with its own event loop + one `httpx.AsyncClient`; GUI producers call `publish_event` which marshals via `call_soon_threadsafe`. Bot token stored in the OS keychain (`gui/telegram_token_store.py`, mirrors `screener/_api_key_store.py`) — never in `users.json` (secret-token rule); enable flag + chat id added to `UserProfile` + `user_store` round-trip. Three emit points in `AppService`: startup→`ToolStartedEvent`, `_on_screener_results_updated`→`ScreenerApprovedEvent(symbols)` (boot reloads excluded), `_refresh_market_status` open→closed→`DayEndPnLEvent`. New System-Settings "Telegram Notifications" section (enable, masked token, chat id, **Send Test**); save reconfigures the worker live. `_start_notifications` guarded so notifications never block startup; `shutdown_notifications` on window close. Added SRD-INF-010.010/.011 (Implemented), MD M08/M09, UTCD M08 (5 cases), RN-INF-1.3.0. pyproject: mypy override for untyped `keyring`. Tests: 5 new gui + 22 notifications = 27 pass; ruff clean on new/changed; mypy --strict clean on the 2 new gui modules (GUI layer has pre-existing graph debt, not held to strict). Doc bumps: SRD-INF 1.8.0, MD-INF 1.3.0, UTCD-INF 1.4.0. **Not committed** — working tree has unrelated pre-existing changes; offered a scoped `feat(inf)` PR. **E2E hand-off:** needs the user's real bot token + chat id entered in Settings → click Send Test (can't test the live Bot API without them). **Remaining follow-ups:** inbound two-way commands (SRD-INF-010.009); per-event UI toggles; scheduled day-end independent of app being open at 16:00 ET. **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown) still pending.

**Previous (Session 71, 2026-06-17):** **Duplicate exit guard triple-layer fix (ISS-EXE-0010, Bugfix, EXE).** On 2026-06-16 during live trading, a position closed via force-exit ■ left a stale pending exit ► signal live in the queue. When that pending exit was executed on the (now-closed) position, it raised `InvalidStateTransitionError`, crashing the engine and corrupting the trade_cycles ledger. Affected WDC/LRCX/TER cycles closed in the 15:44–16:03 window. **Root cause:** Three sequential defects — no pending-signal cleanup on cycle close, no position-state validation before executing a pending exit, and an unguarded state transition on exit fill. **Three-layer fix:** (1) `PendingSignalStore.dismiss_for(strategy_id, symbol, action)` clears matching pending signals; `AppService` subscribes to `CycleClosed` event and calls it with action=EXIT on every cycle close (queue cleanup). (2) `AppService.execute_signal()` validates that a pending EXIT's target cycle is still OPEN before submitting; rejects non-OPEN with WARNING and returns None (execution-time gate). (3) `TradeCycleService.on_exit_fill()` catches `InvalidStateTransitionError` on an exit fill (orphan/duplicate), logs WARNING `[Orders] Orphan/duplicate exit fill ignored`, and returns `None` instead of crashing (fill-time error suppression). New **SRD-EXE-011.024** (pending signal dismissal on cycle close), **SRD-EXE-011.025** (execute_signal OPEN validation), **SRD-EXE-012.014** (on_exit_fill orphan handling). New **DD-EXE-011.024.D01** (three-layer single-exit guard design). New modules: `MD-EXE-011.024.M01` (PendingSignalStore + AppService event wiring); `MD-EXE-012.002.M02` scope extended (on_exit_fill return type → `CycleSnapshot | None`); `MD-EXE-012.002.M04` (protocols updated). Code: 4 files touched (`pending_signal_store.py`, `gui/app_service.py`, `execution/trade_cycle/_service.py`, `execution/trade_cycle/_protocols.py`). Tests: 7 new test_pending_signal_store.py + test_app_service_duplicate_exit.py + test_trade_cycle_service.py cases (all Pass). Artifacts: SRD-EXE 1.21→1.22, DD-EXE 1.15→1.16, MD-EXE 1.13→1.14, UTCD-EXE 1.13→1.14 (+7 tests). RN-EXE-1.30.0-20260617 written; TRACE-EXE 1.19→1.20 (new rows: FO-EXE-011 × 2, FO-EXE-012 × 1, all Implemented, RN filled). ruff + mypy --strict clean on all changed files. **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown); finish remaining UTCD backfill; continue GUI test coverage.

**Previous (Session 70, 2026-06-16):** **ISS-EXE-0009 candle-arming + indicator NaN fix (Bugfix, EXE).** A position opened intraday with manual Buy (strategy SUPERTREND, manual mode) went into loss and never exited, even though its Supertrend exit condition should have fired; the exit only appeared as a pending signal after the tool was closed and reopened. **Root cause:** Two linked defects. (1) Opening a trade cycle armed the tick feed for the symbol but NOT the two candle feeds (historical download + live 3m/15m subscription) that the strategy-condition exit depends on. Those were only re-armed at startup / screener run / reconcile, never on cycle-open. So a held off-screen symbol was evaluated against an empty/sparse candle frame, and the RSI/ATR/SUPERTREND indicators returned NaN. (2) When an indicator returned NaN, the condition `price < NaN` collapsed to `False` with no log, so the un-computable exit silently read as "do not exit". A restart re-downloaded the full day's 1m history, the indicators computed, and the exit fired. **Fix:** (1) **SRD-EXE-006.013** + **DD-EXE-006.013.D01**: `AppService._on_cycle_symbols_changed` now computes just-opened symbols not already covered by the filtered set, emits a new queued `_arm_candle_feeds_requested` signal, and the GUI-thread slot `_arm_candle_feeds` unions them into `_filtered_symbols`, calls `_start_intraday_loader` (delta-fetch, idempotent), and `LiveBarWorker.set_symbols` if a worker is running. Marshalled to GUI thread because the callback fires on the TradeCycleService background thread. (2) **SRD-EXE-011.023**: The `_eval` FUNC branch in `_evaluator.py` now raises `EvaluatorError` when an indicator returns NaN, so an un-computable condition surfaces with a logged WARNING instead of silently reading as false. New module **MD-EXE-006.013.M01** (app_service.py cycle-open arming). Code: 2 files (gui/app_service.py, execution/strategy_engine/_evaluator.py). Tests: UT-EXE-006.013.M01.T01–T04 (off-screen open arms feeds; already-covered no-op; no-worker download-only; signal marshal) + UT-EXE-011.001.M03.T10–T13 (short SUPERTREND/RSI/EMA raise; finite result passes) = 8 new Pass. Artifacts: SRD-EXE 1.20→1.21, DD-EXE 1.14→1.15, MD-EXE 1.12→1.13, UTCD-EXE 1.12→1.13. RN-EXE-1.29.0-20260616 written; TRACE-EXE 1.18→1.19. ruff + mypy --strict clean. **Next:** ISS-EXE-0010 duplicate-exit fix (Session 71).

**Previous (Session 69, 2026-06-12):** **Same-day re-entry monitoring fix (ISS-EXE-0008, Bugfix, EXE).** A completed entry fill for a symbol that was already entered and exited the same day left the `monitoring_session` ledger row in `EXITED` state, blocking the new entry. The reconcile logged `[Lifecycle] Invariant violation during reconcile` for symbols like CCL and SW observed live on 2026-06-12. **Root cause:** `mark_entered` only advanced rows still in `MONITORING` state; once a row transitioned to `EXITED`, a second same-day entry silently no-opped. **Fix:** `mark_entered` now re-arms the most-recent `EXITED` row back to `ENTERED` when no open `MONITORING` row exists, keeping the reconcile invariant (open position ⇒ `ENTERED` row). New helper `fetch_latest_exited_row()` in `_repository.py`; `transition_to_entered()` clears `exited_at`. SRD-EXE-016.007 (Approved → Implemented); DD-EXE-016.007.D01; MD re-updates (016.001.M01, 016.003.M04); UT-EXE-016.007.M01.T01/.T02 (2 new Pass). ISS-EXE-0008 + RN-EXE-1.28.0-20260612 written; TRACE-EXE FO-EXE-016 row added/RN filled. Tests: 24 monitoring_session suite pass; ruff + mypy --strict clean on `_service.py`/`_repository.py`. GUI polish (side-effect): `_ConfirmDialog` close button ✕ now visible (padding reset, font-weight bumped); unaffordable-BUY warning restyled to slim ⚠ + muted line with standard US-broker wording. **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown); FO-EXE-016 UTCD backfill (DoD debt).

**Previous (Session 67, 2026-06-12):** **FO-EXE-017 capital-allocation gaps — global Margin Available ceiling (Feature, EXE + GUI).** Capital allocation only enforced the per-strategy cap (`can_allocate`); the global ceiling (total deployed across all of a user's strategies ≤ Max Capital) was never enforced — the method meant to do it (`can_enter_new`) was dead code. So multiple strategies could each draw a full slice of the same budget, and same-bar entries could collectively over-commit because the gate saw only *filled* positions. Paper mode also reported `open_position_value = 0`, so paper margin and the GUI capital indicator were always wrong, and the live User View never showed Max Capital/Margin Available. **Fix (7 new SRDs .015–.021, all Approved → Implemented):** (1) `RiskManager.margin_available()` = `effective_capital − deployed(all strategies) − reservations`, floored at 0, derived live (never stored). (2) Router blocks entries breaching it (`reason='margin_exhausted'`, edge-triggered warning via new `ctx.margin_warned`). (3) In-flight reservation ledger (`reserve`/`release` on `RiskManager`, added to `RiskValidator` protocol) reserves projected value on commit, released on fill/reject/rollback — kills the same-bar race. (4) Entry qty clamped to `floor(min(strategy_slice, margin_available)/price)`. (5) Paper `get_account_state.open_position_value` now sums open cycles. (6) `AppService.margin_available()` + live `_reconcile_margin_drift()` (one-per-crossing advisory, paper exempt). (7) `_AdminContextBar` single-user capital cell `Cap $X · Avail $Y` (green/red) + mirror SRD-GUI-000.006. Removed dead `can_enter_new`; its 3 tests converted to `margin_available`. Docs: SRD-EXE 1.18→1.19, DD 1.12→1.13, MD 1.10.1→1.11, UTCD 1.10→1.11 (10 new tests, all Pass), GUI SRD 2.12→2.13, TRACE-EXE 1.16→1.17 + TRACE-GUI 1.8→1.9. RN-EXE-1.25.0-20260612. Tests: all capital-allocation + risk-manager suites green, zero new failures (12 pre-existing candle-loader/tick-worker/evaluator failures + 16 `app_service.py` ruff-debt confirmed identical on stashed HEAD); mypy --strict clean on the 4 core EXE files. **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown); FO-EXE-016 UTCD backfill (DoD debt).

**Previous (Session 66, 2026-06-12):** **Three rapid bugfixes in same session:** (1) **ISS-INF-0002 — Paper SimBroker fills MARKET orders at wrong price (Bugfix, INF).** Root cause: SimBroker filled MARKET orders at `OrderRequest.reference_price` (a caller-supplied advisory price computed from the latest 3m candle close) instead of the live market price. A stale/incorrect candle booked a wrong paper fill. Live IBKR broker ignores reference and fills at real `avg_fill_price`. **Fix:** SimBroker accepts an injectable `price_provider(symbol) -> float|None`; MARKET orders now fill at the provider's live price, LIMIT unchanged, fallback to `reference_price/$0` when provider absent or non-positive. `broker_factory` forwards the provider. `AppService` feeds it from a lock-guarded `_last_tick_price` map updated by the live tick stream (GUI thread write, engine thread read). New `SRD-INF-009.007 (Approved → Implemented)`. Tests: 4 new Pass (UT-INF-009.004.M01.T01–T04); 20 broker tests total pass; ruff + mypy clean on sim.py/broker_factory.py. RN-INF-1.1.0-20260612 written; TRACE-INF FO-INF-009 row added/RN filled. Cross-tool: SRD-EXE-015.004 note added (broker_factory forwards price_provider). (2) **ISS-EXE-0006 — Forced exits filled at $0 in paper mode (Bugfix, EXE).** Forced EXIT signals (square-off/end-time/emergency shutdown) omitted `entry_price`, so paper fills recorded a MARKET order at $0, closing cycles at fake ~−100% realized loss. Live-mode exits and normal strategy exits unaffected. **Fix:** New `_open_cycle_price()` helper reads the open cycle's `current_price` when positive, else `entry_price`; `_force_exit` sets entry_price from it. Code review caught a truthiness bug (`current_price or entry_price`) that would let a real 0.0 tick reintroduce $0 fill — corrected to explicit positive guard, locked by test T29. **SRD-EXE-011.022 (new, Approved → Implemented).** Tests: 3 new Pass (T27–T29); 31 total router tests pass; ruff + mypy clean. RN-EXE-1.24.0-20260612 written; TRACE-EXE FO-EXE-011 row RN filled. (3) **ISS-EXE-0005 — Closed-cycle realized P&L multiplied by wrong qty (Bugfix, EXE).** Multi-share positions showed wrong dollar P&L: computed as `(exit_price − entry_price) × exit_qty` where `exit_qty` diverged from held position. A cycle only closes on `FILLED` full-position sell, so realized qty is invariantly `entry_qty`. One-line fix in `_close_cycle()`; SRD-EXE-012.007 re-Implemented. Tests T18 Pass; RN-EXE-1.23.0-20260612 written. **Note:** Screenshot's inflated live prices (CL $322, MNST $403) are a separate price-feed issue — marked for future follow-up. **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown); FO-EXE-016 UTCD backfill (DoD debt).

**Previous (Session 63, 2026-06-10):** **FO-EXE-006.012 — Daily & Weekly Closed Bars for Strategy Conditions (Refactor).** Extended the candle provider to surface stored 1d and 1w closed bars so both entry and exit strategy conditions can evaluate daily/weekly timeframes alongside intraday (1m/3m/5m/1h). Closed bars only — no live aggregation, no merge. (1) **New SRD-EXE-006.012 + DD-EXE-006.012.D01:** requirements for fetching historical 1d/1w bars via DB, serving them to the evaluator. (2) **intraday_candle_loader.py:** new `load_stored_frames()` helper reads 1d/1w from `price_1d`/`price_1w` tables; merged with execution bars in `load_execution_frames()`. (3) **strategy_builder_dialog.py:** added 1d/1w dropdown options so users can write conditions like "close('1d') > ema('1d', 20)". (4) **UTCD:** T17, T18, T19 (3 new tests, all Pass); 7 total candle-readpath tests pass. Pre-existing 12 failures unrelated (verified stashed). Files: `execution/intraday_candle_loader.py`, `gui/strategy_builder_dialog.py`. ruff + mypy clean. RN-EXE-1.21.0-20260610 written; TRACE-EXE updated (new row: FO-EXE-006 SRD-006.012 → DD-006.012.D01 → MD-006.001.M01 → T17–T19 → both files → Implemented). **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown); FO-EXE-016 UTCD backfill (DoD debt).

**Previous (Session 61, 2026-06-09 — follow-up):** **ISS-EXE-0003 — self-healed orphan ledger row logged at ERROR.** After running the tool, the pre-open reconcile logged two `ERROR` lines for `CVS` (a stranded `ENTERED` row with no open position) that was then immediately self-healed to `EXITED`. The heal (PR #36) worked — the errors were just mislabelled. Removed the redundant `entered != keep.carryover` summary block in `reconcile_preopen` (`core/monitoring_session/_service.py`); the per-symbol loop already logs `WARNING` for a healed row and `ERROR` only for the un-healable orphan-open case. New test `UT-EXE-009.002.M02.T17e` (healable orphan → WARNING, no ERROR); 22 service tests pass; ruff + mypy clean. RN-EXE-1.20.1-20260609; ISS-EXE-0003 resolved.

**Previous (Session 61, 2026-06-09):** **FO-EXE-017 — Absolute Capital Allocation, Capital-Max Sizing & Advisory Risk Warnings (full FO→RN chain).** Replaced the percentage-of-equity capital model with an **absolute per-user `$` budget** (`RiskConfig.max_allocation_pct` → `max_capital_value`, default $2000, with one-time settings-JSON migration). Position size is now `floor(effective_capital × strategy.capital_max% / 100 / entry_price)` computed in `_router._size_entry`/`_build_entry_signal` (was hard-coded `qty=1`); `qty<1` drops the entry with a "Capital Max insufficient" warning. Wired the real `RiskManager` into the engine (replacing the no-op `PassthroughRiskValidator`): budget-based `can_allocate` (blocking), advisory `validate_signal` (only the circuit breaker blocks; Max Position / Risk-per-trade emit `RiskWarning` events, never block). `effective_capital()` on `AppService`: paper = stored Max Capital (also seeds paper `AccountState.equity`); live = compare vs broker cash on connect → if Max Capital > cash, one LIVE-LOG warning + use 90% of cash (stored setting untouched). Daily-loss aggregated across all active trades → advisory `RiskWarning` with one-per-crossing latch, bridged to `risk_warning_raised` + a debounced non-blocking `QMessageBox` in `main_window` + Live Log. **Rex fixes:** auto-reset on `STOPPED→RUNNING` in `_engine._apply_run_state`; Active Trades Rex column now shows remaining re-entries (never `-1`) and suppresses an open cycle's exhausted counter on a pending duplicate row (root cause of the user's `-1`-on-pending report). 13 source modules; new `RiskWarning` event; `RiskManager` ctor gains `effective_capital_provider` + `warning_sink` (back-compatible defaults). Docs: FO-EXE-017 + SRD-EXE-017.001–014 (Implemented) + DD/MD/UTCD (22 tests, all Pass) + SRD-GUI-014.013 Reopen; TRACE-EXE 1.14→1.15; RN-EXE-1.20.0-20260609. Tests: 22 new pass; full execution+gui suite 204 pass / 21 pre-existing failures (verified identical with changes stashed); ruff + mypy --strict clean on changed files. Merged via PR #37 (branch carried the pre-existing Session-60 ISS-EXE-0002 work, renumbered RN-1.19.0→1.19.1 to avoid collision with main's PR #36 orphaned-ledger RN-1.19.0). **Next:** FO-EXE-003 (CircuitBreaker + EmergencyShutdown); FO-EXE-016 UTCD backfill (DoD debt).

**Previous (Session 60, 2026-06-08):** **Fixed orphaned ENTERED ledger rows in monitoring-session lifecycle (hotfix for FO-EXE-009/010).** Root cause: trade-cycle closes via non-FILLED exit (abort, manual close) did not flip the ledger state, leaving symbols stranded in ENTERED. Observed live for symbol SATS on 2026-06-08. Solution: (1) New `wire_cycle_ledger_projection(bus, command, terminal_event_types, *, clock=None)` factory in `core/monitoring_session/__init__.py` — wires a stateless handler that subscribes to injected terminal trade-cycle events (CycleClosed, CycleAborted) and calls `mark_exited(symbol, now)` on each. (2) `reconcile_preopen()` in `_service.py` now self-heals orphaned ENTERED rows (entered but no open cycle) by calling `mark_exited()` instead of reporting invariant violation — logged as heal warning so user can investigate if unexpected. (3) Wired in `gui/app_service.py` after lifecycle service construction. Tests: UT-EXE-009.002.M02.T17 rewritten (heal instead of report) + 3 new cases T17b–T17d; suite 79 pass. ruff + mypy --strict clean. RN-EXE-1.19.0-20260608 written (PR #36). **Next:** UTCD backfill for FO-EXE-016 (Session 59 DoD debt); FO-EXE-003 (CircuitBreaker + EmergencyShutdown).

**Also (Session 60, 2026-06-08):** **Fixed ISS-EXE-0002 — manual order fills dropped as "unknown order" due to cross-thread race (Active Trades Play/buy, force-exit).** Root cause: order context stored by `broker_order_id` **after** `place_order` had already scheduled the fill on the engine thread. Fill arrived first → context lookup failed → dropped. **Fix:** register context **before** placing order, keyed by `client_ref` (signal_id) which every broker echoes on every `OrderEvent`, making it broker- and thread-agnostic. `OrderIngestion._context` now keyed by `client_ref`; `register()` pre-placement; `on_order_accepted()` backfills broker id and inserts NEW trades row (idempotent); `on_order_event()` resolves by `event.client_ref`; `on_conflict_do_nothing()` in `insert_trade` tolerates fill-before-acceptance race. 3 changed modules + 1 regression test; 171 suite pass; ruff/mypy clean. RN-EXE-1.19.1-20260608 (renumbered from 1.19.0); ISS-EXE-0002 resolved. Landed via PR #37.
...
| Artifact | File | Status | Notes |
|---|---|---|---|
| FO | `docs/execution/FO.md` | Draft v1.7.0 | FO-EXE-011 (Strategy Engine) and FO-EXE-012 (Trade Cycle Ledger) both Approved; FO-EXE-011 now Verified with refactored lifecycle model; FO-EXE-015 (Broker Adapter) Implemented (ISS-EXE-0002 cross-thread race fix) |
| SRD | `docs/execution/SRD.md` | Draft v1.7.0 | SRD-EXE-015.001–006 Implemented (race-free context registration); 25 Implemented across other FOs, 60 Draft/Approved |
...
| MD | `docs/execution/MD.md` | Draft v1.6.0 | MD-EXE-015.001–003 (order_ingestion, broker_adapter, broker_factory modules); 26 modules total |
| UTCD | `docs/execution/UTCD.md` | Draft v1.2.0 | Tests for FO-EXE-011/012: 67 cases (38+29) all Pass; test_fill_arriving_before_acceptance_is_not_dropped regression added; 142 total cases specified |
| TRACE | `docs/execution/TRACE.md` | Draft v1.14.0 — **updated** | FO-EXE-015 row added (Implemented, RN-EXE-1.19.0-20260608); Last Updated 2026-06-08 |
